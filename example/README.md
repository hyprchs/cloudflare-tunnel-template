...
writes `example/.env` with your tunnel token (and service token credentials when created), and writes an `example/.env.example` file.
...
b.
  ...
  - Update `example/.env` with `CF_ACCESS_CLIENT_ID=<client id>` and `CF_ACCESS_CLIENT_SECRET=<client secret>`.
